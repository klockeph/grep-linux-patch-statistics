...
# returns if the sub-version is divisible by 10
subversion.div10 <- function(x) {
	version <- strsplit(x, '\\.')[[1]]
	if (length(version) < 3) {
		return(TRUE)
	}
	subversion <- strtoi(version[3])
	return(subversion %% 10 == 0)
}

label_if_tenth <- function(x) {
	if (subversion.div10(x))
		return(x)
	return("")
}

# initialize data frame
data <- data.frame(version=character(), timestamp=numeric(), patches=numeric(), lts_version=character())
...
for (filename in args) {
	...
	# only keep every tenth version
	df$version <- vapply(df$version, label_if_tenth, "X", USE.NAMES=FALSE)
	data <- rbind(data, df)
}
...
p <-ggplot(data=data, aes(x=timestamp, y=patches, color=lts_version)) +
	geom_point() +
	geom_label_repel(aes(label=version)) +                                                    # labels
	scale_x_datetime(date_breaks='1 month', limits=c(as.POSIXct('2015-01-01'), Sys.time())) + # x ticks
	xlab("Versions") +
	ylab("Patches") +
...
ggsave("patches_LTS.png", plot=p, width=50, height=20, units="cm")
